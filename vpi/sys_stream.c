...
static void send_message(char* msg) {
	int key_len = strlen(stream_key);
	int msg_len = strlen(msg);
	rd_kafka_resp_err_t err;
	err = rd_kafka_producev(producer,
		RD_KAFKA_V_TOPIC(topic),
		RD_KAFKA_V_MSGFLAGS(RD_KAFKA_MSG_F_COPY),
		RD_KAFKA_V_KEY((void*)stream_key, key_len),
...
static inline int dump_header_pending(void) {
...
static PLI_INT32 stream_finish_cb(p_cb_data cause) {
	...
	if (dumpvars_time != stream_cur_time && !dump_is_off) {
...
static void show_this_item_x(struct stream_info *info) {
	PLI_INT32 type = vpi_get(vpiType, info->item);

	if (type == vpiRealVar) {
		strbuf_append_linef(&message, "rNan %s\n", info->ident);
	} else if (type == vpiNamedEvent) {
		
	} else if (vpi_get(vpiSize, info->item) == 1) {
		strbuf_append_linef(&message, "x %s\n", info->ident);
	} else {
		strbuf_append_linef(&message, "bx %s\n", info->ident);
	}
}
...
__inline__ static int install_dumpvars_cb(void) {
	struct t_cb_data cb;
	
	if (stream_status == 1) return 0;
	if (stream_status == 2) {
		vpi_printf("Stream warning: $dumpvars ignored, previously called at simtime %" PLI_UINT64_FMT "\n", dumpvars_time);
		return 1;
	}
...
static void start_producer(vpiHandle callh) {
	...
	if (!producer) {
		...
		stream_key = asctime(localtime(&walltime));
...
static void scan_item(unsigned depth, vpiHandle item, int skip) {
	...
	switch (item_type) {
	  ...
	  default:
	    vpi_printf("Streaming warning: $dumpvars: Unsupported argument "
	...
	case vpiPackage:
		if (vcd_instance_contains_dumpable_items(dumpable_types, item)) {
			vpi_printf("Stream warning: $dumpvars: Package (%s) is not dumpable with stream.\n", vpi_get_str(vpiFullName, item));
		}
		break;
...
static int draw_scope(vpiHandle item, vpiHandle callh) {
	...
	switch (vpi_get(vpiType, scope)) {
		...
		default:
			type = "invalid";
			vpi_printf("Stream error: %s:%d: $dumpfile: Unsupported scope type (%d)\n",
...
static PLI_INT32 sys_dumpall_calltf(ICARUS_VPI_CONST PLI_BYTE8 *name) {
	...
	if (now64 > stream_cur_time) {
		strbuf_append_linef(&message, "#%" PLI_UINT64_FMT "\n", now64);
		stream_cur_time = now64;
	}
...
static PLI_INT32 sys_dumplimit_calltf(ICARUS_VPI_CONST PLI_BYTE8 *name) {
	(void)name;
	printf("WARNING: dumplimit has no effect on streaming");
	return 0;
}
...
static PLI_INT32 sys_dumpon_calltf(ICARUS_VPI_CONST PLI_BYTE8 *name) {
    ...
    strbuf_append_linef(&message, "$dumpon\n");
    ITERATE_VCD_INFO(stream_list, stream_info, next, show_this_item);
    strbuf_append_linef(&message, "$end\n");
	return 0;
}
...
static PLI_INT32 sys_dumpoff_calltf(ICARUS_VPI_CONST PLI_BYTE8 *name) {
	...
	strbuf_append_linef(&message, "$dumpoff\n");
	ITERATE_VCD_INFO(stream_list, stream_info, next, show_this_item_x);
	strbuf_append_linef(&message, "$end\n");
	send_message(message.data);
	strbuf_free(&message);
	return 0;
